...
$.fn.alignToElement = function(refElement, alignment){
...
$.fn.createDropdownOptions = function(attrNames){
    var ddMenu = $(this).find('ul.dropdown-menu');
    if(Array.isArray(attrNames)){
        attrNames.forEach(function(attrName, i){
            $("<li role='presentation'><a href='#' role='menuitem' tabindex='" + (i+1) + "'>" + attrName + "</a></li>").appendTo(ddMenu);
        });
    }
    else{
        $('<li>Error</li>').appendTo(ddMenu);
    }
